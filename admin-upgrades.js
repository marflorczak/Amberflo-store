...
function installStoreManagement(){
  ...
  content.insertAdjacentHTML('beforeend',`<div class="panel" id="shippingPanel"><div class="panel-head"><div><p class="eyebrow">Ustawienia sklepu</p><h1>Dostawa</h1><p>Zmieniaj ceny i dostępność metod dostawy.</p></div></div><div class="shipping-admin-list" id="shippingAdminList"></div></div><div class="panel" id="ordersPanel"><div class="panel-head"><div><p class="eyebrow">Obsługa sprzedaży</p><h1>Zamówienia</h1><p>Kontroluj płatności, status i wysyłaj klientom dane do śledzenia.</p></div></div><div class="admin-note">Bez własnej domeny przycisk wysyłki przygotuje gotową wiadomość w Twoim programie pocztowym. Po włączeniu domeny Resend e-mail zostanie wysłany automatycznie.</div><div class="orders-admin-list" id="ordersAdminList"></div></div>`);
}
...
function renderOrderItems(order){return (order.items||[]).map(item=>`<li>${escapeHtml(item.name||item.id)} × ${item.quantity||item.qty||1} — ${adminMoney((item.price||0)*(item.quantity||item.qty||1))}</li>`).join('')}
...
async function loadAdminOrders(){
  ...
  try{const rows=await api('/rest/v1/orders?select=*&order=created_at.desc');document.querySelector('#orderCount').textContent=rows.length;list.innerHTML=rows.map(order=>{const customer=orderCustomer(order);const shipping=order.shipping_method||{};const point=customer.inpost_point_name?`<p><b>Paczkomat</b>${escapeHtml(customer.inpost_point_name)}${customer.inpost_point_address?`<br>${escapeHtml(customer.inpost_point_address)}`:''}</p>`:'';return `<article class="order-card" data-order-id="${escapeHtml(order.id)}"><div class="order-card-head"><div><h3>#${escapeHtml(order.id.slice(0,8).toUpperCase())} · ${escapeHtml(customer.name||'Klient')}</h3><p>${new Date(order.created_at).toLocaleString('pl-PL')} · ${escapeHtml(customer.email||'')} · ${escapeHtml(customer.phone||'')}</p></div><span class="order-status">${escapeHtml(orderStatusLabels[order.status]||order.status)}</span></div><div class="order-details"><p><b>Adres</b>${escapeHtml(customer.address||'').replace(/\n/g,'<br>')}</p>${point}<p><b>Dostawa</b>${escapeHtml(shipping.name||shipping.name_pl||'—')} · ${adminMoney(order.shipping_cost||0)}</p><p><b>Razem</b>${adminMoney(order.total||0)}</p><p><b>Uwagi</b>${escapeHtml(customer.notes||'—')}</p></div><ul class="order-items">${renderOrderItems(order)}</ul><div class="order-actions"><label>Status<select data-order-status>${Object.entries(orderStatusLabels).map(([value,label])=>`<option value="${value}" ${order.status===value?'selected':''}>${label}</option>`).join('')}</select></label><label>Numer przesyłki<input data-tracking-number value="${escapeHtml(order.tracking_number||'')}"></label><label>Link do śledzenia<input data-tracking-url type="url" value="${escapeHtml(order.tracking_url||'')}"></label><div><button class="save-status" data-save-order="${escapeHtml(order.id)}">Zapisz status</button> <button class="ship-button" data-ship-order="${escapeHtml(order.id)}">Oznacz jako wysłane</button></div></div></article>`}).join('')||'<div class="empty-admin">Nie ma jeszcze zamówień.</div>'}
  catch(error){list.innerHTML=`<div class="empty-admin">${escapeHtml(error.message)}. Uruchom plik supabase/shipping-orders-upgrade.sql.</div>`}
}
