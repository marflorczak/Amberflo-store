...
serve(async req => {
  ...
  try {
    ...
    const text = `Dzień dobry ${customer.name || ""},\n\nTwoje zamówienie Amberflo #${orderShort} zostało anulowane.${reasonText}\nJeśli zamówienie zostało opłacone, zwrot płatności zostanie przekazany przez Stripe tą samą metodą, której użyto podczas zakupu. Środki zwykle pojawiają się na koncie w ciągu 5–10 dni roboczych. Dokładny czas zależy od banku lub operatora płatności.\n\nW razie pytań prosimy o kontakt: biuroamberflo@gmail.com\n\nAmberflo`;
    const html = `<h1>Zamówienie zostało anulowane</h1><p>Dzień dobry ${esc(customer.name)},</p><p>Zamówienie <b>#${orderShort}</b> zostało anulowane.</p>${reasonHtml}<p>Jeśli zamówienie zostało opłacone, zwrot płatności zostanie przekazany przez <b>Stripe</b> tą samą metodą, której użyto podczas zakupu. Środki zwykle pojawiają się na koncie w ciągu <b>5–10 dni roboczych</b>. Dokładny czas zależy od banku lub operatora płatności.</p><p>W razie pytań prosimy o kontakt: <a href="mailto:biuroamberflo@gmail.com">biuroamberflo@gmail.com</a></p><p>Amberflo</p>`;
